...
func (m *Memberlist) gossip() {
	...
	toSelectNodes := m.neighbors
	if !m.config.IsMgossip {
		toSelectNodes = m.nodes
	}
	m.nodeLock.RLock()
	kNodes := kRandomNodes(m.config.GossipNodes, toSelectNodes, func(n *nodeState) bool {
...
func (m *Memberlist) pushPull() {
	...
	nodes := kRandomNodes(1, toSelectNodes, func(n *nodeState) bool {
		return n.Name == m.config.Name ||
			n.State != StateAlive
	})
	m.nodeLock.RUnlock()
